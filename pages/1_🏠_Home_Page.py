...
def load_data_from_duckdb():
    ...
    df = con.execute("SELECT * FROM duckdb_databases").df()
    con.close()
    return df


#duck_con = load_data_from_duckdb()
# เชื่อมกับฐาน condo.duckdb
con = duckdb.connect("condo_sql.duckdb")
# รันคำสั่งเพื่อดูรายชื่อตารางทั้งหมด
...
def main_page():
    ...
    #df1 = pd.read_sql(final_sql_query, duck_con)
    #df1 = duck_con.execute(final_sql_query).df()

    # Register the DataFrame as a DuckDB table
    #duck_con.register('condo', df1)
    #result = duck_con.execute("SELECT * FROM condo").df()
    #result = duck_con.execute(final_sql_query).df()
    #duck_con.execute("SHOW TABLES").df()

    con = duckdb.connect("condo_sql.duckdb")
    df1 = pd.read_sql(final_sql_query, con)
    result = con.execute(final_sql_query).df()
